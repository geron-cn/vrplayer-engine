...
void Game::setDesiredFPS(float fps)
{
    if (fps > 0)
        _updateInterval = 1000.f / fps;
}
...
void Game::frame()
{
    ...
    if (frameTime - lastFrameTime < _updateInterval)
    {
        long sleeptime = (long)(_updateInterval - frameTime + lastFrameTime);
        if (sleeptime > 0)
        {
