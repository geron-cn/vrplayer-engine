...
static const ASensor* __rotationVectorSensor;
...
static float __rotationX;
static float __rotationY;
static float __rotationZ;
static float __rotationW;
...
static void engine_handle_cmd(struct android_app* app, int32_t cmd)
{
    switch (cmd) 
    {
        ...
        case APP_CMD_LOST_FOCUS:
            ...
            if (__rotationVectorSensor != NULL)
            {
                ASensorEventQueue_disableSensor(__sensorEventQueue, __rotationVectorSensor);
            }
            break;
        case APP_CMD_GAINED_FOCUS:
            ...
            if (__rotationVectorSensor != NULL)
            {
                ASensorEventQueue_enableSensor(__sensorEventQueue, __rotationVectorSensor);
                // We'd like to get 60 events per second (in microseconds).
                ASensorEventQueue_setEventRate(__sensorEventQueue, __rotationVectorSensor, (1000L/60)*1000);
            }
...
int Platform::enterMessagePump()
{
    ...
    __rotationVectorSensor = ASensorManager_getDefaultSensor(__sensorManager, 11);
    ...
    while (true)
    {
        ...
        while ((ident=ALooper_pollAll(!__suspended ? 0 : -1, NULL, &events, (void**)&source)) >= 0) 
        {
            ...
            if (ident == LOOPER_ID_USER && __accelerometerSensor != NULL)
            {
                ...
                else if (__sensorEvent.type == 11)
                {
                    __rotationX = __sensorEvent.data[0];
                    __rotationY = __sensorEvent.data[1];
                    __rotationZ = __sensorEvent.data[2];
                    __rotationW = __sensorEvent.data[3];
                }
            }
...
void Platform::getAccelerometerValues(float* pitch, float* roll)
{
    ...
    static Matrix getRotateEulerMatrix(float x, float y, float z)
    {
        x *= (float)(M_PI / 180.0f);
        y *= (float)(M_PI / 180.0f);
        z *= (float)(M_PI / 180.0f);
...
gameplay::Matrix Platform::getHeadViewMatrix()
{
    gameplay::Quaternion q(__rotationX, __rotationY, __rotationZ, __rotationW);
    Matrix inertialReferenceFrameToDevice;
    q.w = -q.w;
    Matrix::createRotation(q, &inertialReferenceFrameToDevice);
    
    static Matrix worldToInertialReferenceFrame = getRotateEulerMatrix(-90.f, 0.f, 90.f);
    static Matrix deviceToDisplay = getRotateEulerMatrix(0.f, 0.f, -90.f);
    
    Matrix worldToDevice;
    Matrix::multiply(inertialReferenceFrameToDevice, worldToInertialReferenceFrame, &worldToDevice);
    
    Matrix worldToDisplay = (deviceToDisplay * worldToDevice);
