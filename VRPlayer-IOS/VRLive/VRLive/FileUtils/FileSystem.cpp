...
public:
    ...
    virtual void close();
    virtual size_t read(void* ptr, size_t size, size_t count);
    virtual char* readLine(int num, char* str);
    virtual size_t write(const void* ptr, size_t size, size_t count);
    virtual bool eof();
    virtual size_t length();
    virtual long int position();
...
char* FileStream::readLine(int num, char* str)
{
    if (!_file)
        return 0;
...
char* FileStreamAndroid::readLine(int num, char* str)
{
    if (num <= 0)
        return NULL;
    char c = 0;
...
long int FileStreamAndroid::tell()
{
    return position();
}
