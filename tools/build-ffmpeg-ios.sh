...
PRODUCT_DIR=$SCRIP_DIR/products/ios
...
do_lipo_ffmpeg () {
    ...
    do
        ARCH_LIB_FILE="$UNI_BUILD_ROOT/ffmpeg-$ARCH/lib/$LIB_FILE"
    ...
    xcrun lipo -create $LIPO_FLAGS -output $UNI_BUILD_ROOT/universal/lib/$LIB_FILE
    xcrun lipo -info $UNI_BUILD_ROOT/universal/lib/$LIB_FILE
}
...
do_lipo_ssl () {
    ...
    for ARCH in $FF_ALL_ARCHS
    do
        ARCH_LIB_FILE="$UNI_BUILD_ROOT/openssl-$ARCH/lib/$LIB_FILE"
        if [ -f "$ARCH_LIB_FILE" ]; then
            LIPO_FLAGS="$LIPO_FLAGS $ARCH_LIB_FILE"
        else
            echo "skip $LIB_FILE of $ARCH";
...
do_lipo_all () {
    mkdir -p $UNI_BUILD_ROOT/universal/lib
    ...
    for ARCH in $FF_ALL_ARCHS
    do
        ARCH_INC_DIR="$UNI_BUILD_ROOT/ffmpeg-$ARCH/include"
        if [ -d "$ARCH_INC_DIR" ]; then
            if [ -z "$ANY_ARCH" ]; then
                ANY_ARCH=$ARCH
                cp -R "$ARCH_INC_DIR" "$UNI_BUILD_ROOT/universal/"
            fi

            UNI_INC_DIR="$UNI_BUILD_ROOT/universal/include"
...
if [ "$FF_TARGET" = "armv7" -o "$FF_TARGET" = "armv7s" -o "$FF_TARGET" = "arm64" ]; then
    echo_archs
    sh $SCRIP_DIR/do-compile-ffmpeg_ios.sh $FF_TARGET
    do_lipo_all
    ...
    for ARCH in $FF_ALL_ARCHS
    do
        sh $SCRIP_DIR/do-compile-ffmpeg-ios.sh $ARCH
    done
    ...
    echo "Usage:"
    echo "  bulid-ffmpeg-ios.sh armv7|arm64|i386|x86_64"
    echo "  bulid-ffmpeg-ios.sh armv7s (obselete)"
    echo "  bulid-ffmpeg-ios.sh lipo"
    echo "  bulid-ffmpeg-ios.sh all"
    echo "  cbulid-ffmpeg-ios.sh clean"
    echo "  cbulid-ffmpeg-ios.sh check"
    exit 1
fi
